...
<body class="font-sans antialiased bg-zinc-50">
    @php
        ...
        if (str_contains($routeName, 'dashboard')) {
            ...
            $activeMenu = 'sales';
        } elseif (str_contains($routeName, 'kawanhijau-ai')) {
            $activeMenu = 'kawanhijau-ai';
    ...
    <div class="min-h-screen flex flex-col lg:flex-row" x-data="sidebar">
        <!-- Mobile/Tablet Top Header -->
        <header class="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-zinc-200 shadow-sm">
            <div class="flex items-center justify-between h-16 px-4">
                <!-- Hamburger Button -->
                <button @click="sidebarOpen = !sidebarOpen" class="p-2 rounded-lg hover:bg-zinc-100 transition-colors">
        ...
        <div x-show="sidebarOpen" x-cloak x-transition:enter="transition-opacity ease-out duration-300"
            ...
            class="lg:hidden fixed inset-0 bg-zinc-900/50 backdrop-blur-sm z-40">
        </div>
        ...
        <aside x-show="sidebarOpen" x-cloak
            ...
            class="fixed lg:sticky left-0 top-0 h-screen w-64 bg-white border-r border-zinc-200 shadow-xl lg:shadow-none z-50 flex flex-col">
            ...
            <nav class="flex-1 overflow-y-auto px-4 py-4 space-y-1">
                ...
                @elseif(auth()->user()->hasRole('petani'))
                    ...
                    <a href="{{ route('petani.kawanhijau-ai') }}"
                        class="flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 group {{ $activeMenu === 'kawanhijau-ai' ? 'bg-gradient-to-r from-primary-500 to-secondary-500 text-white shadow-md' : 'text-zinc-700 hover:bg-zinc-100' }}">
                        <svg class="w-5 h-5 {{ $activeMenu === 'kawanhijau-ai' ? 'text-white' : 'text-zinc-600 group-hover:text-primary-600' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                        </svg>
                        <span class="font-medium text-sm">KawanHijau AI</span>
                    </a>
